...
// conseguiria usar o Konekto como proxy pra bater em infraestrutura interna
  ...
  for (let redirectCount = 0; redirectCount <= MAX_REDIRECTS; redirectCount++) {
    ...
    try {
      response = await fetch(parsed.toString(), {
        method: 'POST',
        headers: { 'content-type': 'application/json', 'X-Konekto-Signature': `sha256=${signature}` },
        body,
        redirect: 'manual',
...
/// `X-Konekto-Signature`, mesma convenção do GitHub/Stripe, pro lado
/// receptor poder confirmar que a chamada veio mesmo do Konekto.
///
