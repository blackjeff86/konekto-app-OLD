...
const List<String> _kConfigSections = ['Marca', 'Serviços', 'Equipe'];
...
class SettingsPage extends StatefulWidget {
  ...
  const SettingsPage({super.key, required this.session, required this.authRepository});
...
class _SettingsPageState extends State<SettingsPage> {
  ...
  Widget build(BuildContext context) {
    if (!_isGerente) {
      return Center(
        child: Text('Só gerentes têm acesso a Configurações.', style: KonektoBrand.body(fontSize: 14)),
      );
    }
      ...
      children: [
        ...
        Expanded(
          child: switch (_selectedSection) {
            0 => _BrandingSection(session: widget.session, authRepository: widget.authRepository),
            1 => ServicesListPage(session: widget.session, authRepository: widget.authRepository),
            2 => InviteStaffPage(session: widget.session, authRepository: widget.authRepository),
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: selected ? KonektoBrand.gold.withValues(alpha: 0.14) : Colors.transparent,
          borderRadius: BorderRadius.circular(999),
          border: Border.all(color: selected ? KonektoBrand.gold.withValues(alpha: 0.5) : KonektoBrand.borderStrong),
        ),
        child: Text(
          label,
...
class _BrandingSectionState extends State<_BrandingSection> {
  ...
  Future<void> _loadConfig() async {
    try {
      ...
      final colorPalette = config['colorPalette'] as Map<String, dynamic>? ?? {};
      ...
      child: ConstrainedBox(
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              Text('Marca do hotel', style: KonektoBrand.display(fontSize: 18)),
              const SizedBox(height: 4),
              Text('Nome, logo e cores usados no app do hóspede.', style: KonektoBrand.body(fontSize: 12.5)),
              const SizedBox(height: 24),
              ...
              _SettingsField(label: 'Nome do hotel', controller: _nameController),
              const SizedBox(height: 14),
              _SettingsField(label: 'URL do logo', controller: _logoUrlController),
              const SizedBox(height: 14),
              _SettingsField(label: 'Cor primária (hex)', controller: _primaryController),
              const SizedBox(height: 14),
              _SettingsField(label: 'Cor secundária (hex)', controller: _secondaryController),
              const SizedBox(height: 24),
              SizedBox(
